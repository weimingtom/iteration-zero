/*
  ---------------------------------------------------------------------
  luigi/example1_gld.d -- An example program using the Luigi user
                      interface library with the GLD windowing toolkit.
                     
  An example/test program for Luigi.
  Uses the GLD windowing toolkit via luigi.adapter.gld.

  This example is almost identical to example1, just it uses GLD instead
  of GLFW.
...
luigui.Overlay create_gui(char[] theme)
{
    luigui.Overlay gui;
    with(luigui) {
        Luigi().adapter = luigi.adapters.gld.GLDAdapter();
...
void main(char[][] argv)
{
    ...
    //---------------- SETUP -----------------------
    // Open OpenGL window
    if (!gldOpenWindow( 640, 480, 0,0,0,0, 0,0, GLD_WINDOW ))
        return;

    // Enable sticky keys
    gldEnable( GLD_STICKY_KEYS );
    gldEnable( GLD_KEY_REPEAT );

    // Disable vertical sync if requested (on cards that support it)
    if (!dosync) gldSwapInterval( 0 );
    ...
    real show_fps() 
    {
        ...
        if( (t-t0) > 1.0 || frames == 0 )
        {
            ...
            gldSetWindowTitle( titlestr.ptr );
            t0 = t;
            frames = 0;
        }
        frames ++;
    ...
    while (running)
    {
        ...
        running = 
            !gldGetKey( GLD_KEY_ESC ) &&
            gldGetWindowParam( GLD_OPENED );
    }
